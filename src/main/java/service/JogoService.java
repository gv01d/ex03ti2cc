...
public class JogoService {
	...
	public void makeForm(int tipo, Jogo jogo, int orderBy) {
		...
		String umJogo = "";
		if(tipo != FORM_INSERT) {
			umJogo += "\t<table width=\"100%\" bgcolor=\"#f3f3f3\" align=\"center\">";
			umJogo += "\t\t<tr>";
		...
		if(tipo == FORM_INSERT || tipo == FORM_UPDATE) {
			...
			if (tipo == FORM_INSERT){
				action += "insert";
				name = "Inserir Jogo";
				descricao = "Breath Of The Wild, Apex, ...";
				buttonLabel = "Inserir";
			} else {
			...
			umJogo += "\t<table class=\"tab1\" width=\"100%\" bgcolor=\"#f3f3f3\" align=\"center\">";
			umJogo += "\t\t<tr>";
			...
			umJogo += "\t\t</tr>";
			umJogo += "\t\t<tr>";
			umJogo += "\t\t\t<td>&nbsp;Descrição: <input class=\"input--register\" type=\"text\" name=\"descricao\" placeholder=\""+ descricao +"\"></td>";
			...
			umJogo += "\t\t</tr>";
			umJogo += "\t\t<tr>";
			umJogo += "\t\t\t<td>&nbsp;Data de fabricação: <input class=\"input--register\" type=\"text\" name=\"dataFabricacao\" value=\""+ jogo.getDataFabricacao().toString() + "\"></td>";
			umJogo += "\t\t\t<td align=\"center\"><input type=\"submit\" value=\""+ buttonLabel +"\" class=\"input--main__style input--button\"></td>";
			umJogo += "\t\t</tr>";
			umJogo += "\t</table>";
			umJogo += "\t</form>";		
		} else if (tipo == FORM_DETAIL){
			umJogo += "\t<table class=\"tab2\" width=\"100%\" bgcolor=\"#f3f3f3\" align=\"center\">";
			umJogo += "\t\t<tr>";
			...
			umJogo += "\t\t</tr>";
			umJogo += "\t\t<tr>";
			umJogo += "\t\t\t<td>&nbsp;Data de fabricação: "+ jogo.getDataFabricacao().toString() + "</td>";
			umJogo += "\t\t\t<td>&nbsp;</td>";
			umJogo += "\t\t</tr>";
			umJogo += "\t</table>";		
		} else {
		...
		String list = new String("<table class=\"tab2\" width=\"100%\" align=\"center\" bgcolor=\"#f3f3f3\">");
		list += "\n<tr><td colspan=\"6\" align=\"left\"><font size=\"+2\"><b>&nbsp;&nbsp;&nbsp;Relação de Jogos</b></font></td></tr>\n" +
        		...
        		"\t<td><a href=\"/jogo/list/" + FORM_ORDERBY_DESCRICAO + "\"><b>Descrição</b></a></td>\n" +
		...
		} else if (orderBy == FORM_ORDERBY_DESCRICAO) {		jogos = jogoDAO.getOrderByDescricao();
		...
		for (Jogo p : jogos) {
			...
			list += "\n<tr class=\"out\">\n" + 
            		  ...
            		  "\t<td align=\"center\" valign=\"middle\"><a href=\"javascript:confirmarDeleteJogo('" + p.getID() + "', '" + p.getDescricao() + "', '" + p.getPreco() + "');\"><img src=\"/image/delete.png\" width=\"20\" height=\"20\"/></a></td>\n" +
            		  "</tr>\n";
		}
		list += "</table>";		
	...
	public Object insert(Request request, Response response) {
		...
		Jogo jogo = new Jogo(-1, descricao, preco, quantidade, dataFabricacao);
		
		if(jogoDAO.insert(jogo) == true) {
            resp = "Jogo (" + descricao + ") inserido!";
            response.status(201); // 201 Created
		} else {
			resp = "Jogo (" + descricao + ") não inserido!";
	...
	public Object update(Request request, Response response) {
        ...
        if (jogo != null) {
        	...
        	jogo.setDataFabricacao(LocalDateTime.parse(request.queryParams("dataFabricacao")));
        	jogoDAO.update(jogo);
        	response.status(200); // success
